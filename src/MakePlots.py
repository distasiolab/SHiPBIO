...
filename = os.path.join(FILEPATHBASE, 'calc', 'samples_all_integrated_imputed_PHATE_clustered.h5ad')
...
# Plotting
n_clusters = len(np.unique(samples_all.obs['PHATE_cluster']))

GroupsToAnalyze = list(np.arange(0,n_clusters))
groups = [sorted(np.unique(samples_all.obs['PHATE_cluster']), key=lambda x: int(x))[i] for i in GroupsToAnalyze]
...
if SAVEFIGS:
    filename_out = os.path.join(IMGDIR, 'AllSamples_PHATE_' + str(n_clusters) + '_Clusters.png')
    fig.savefig(filename_out, dpi=300)
    print('Saved: ' + filename_out)
    filename_out = os.path.join(IMGDIR, 'AllSamples_PHATE_' + str(n_clusters) + '_Clusters.svg')
    fig.savefig(filename_out, dpi=300)
    print('Saved: ' + filename_out)
