...
# --------------------------------------------------------------------------------
# Remove clusters that have contribution from <3 samples
# --------------------------------------------------------------------------------
print('Removing clusters with contribution from <3 samples')
groups = []
for cl in set(retinas_all.obs['leiden']):
    SamplesThisCluster = retinas_all.obs['batch'][retinas_all.obs['leiden'] == cl]
    nSamplesPerCluster = len(set(retinas_all.obs['batch'][retinas_all.obs['leiden'] == cl]))
    if nSamplesPerCluster > 2:
        groups.append(cl)

retinas_all = retinas_all[retinas_all.obs['leiden'].isin(groups)]
...
if SAVEFIGS:
    ...
    # Only show the first 10 clusters
    groups = sorted(set(retinas_all.obs['leiden']), key=lambda x: int(x))[0:10]
